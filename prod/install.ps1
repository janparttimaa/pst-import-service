...
.RELEASE NOTES
    ...
    20250607 - Updated monthlycleanup.ps1 generation to include 30-day folder retention logic instead of clearing contents monthly.
#>
...
Remove-Item -Path "$driveLetter\$tools\$($extractedFolder.Name)" -Force -Recurse -Verbose
...
# Create monthlycleanup.ps1 with 30-day retention logic
...
New-Item -Path $targetScriptPath -ItemType File -Force | Out-Null
Set-Content -Path $targetScriptPath -Value $scriptContent -Force
...
$description = "This task will make sure that hidden folder \"PST Import Service$\" will be cleaned based on 30-day retention."
